...
const serp_1 = __importDefault(__webpack_require__(/*! ../serp */ "./src/serp.ts"));
const quizlet_1 = __webpack_require__(/*! ../quizlet */ "./src/quizlet.ts");
const fuzzy_1 = __webpack_require__(/*! ../fuzzy */ "./src/fuzzy.ts");
//
...
async function handleClick(info, tab) {
    ...
    const results = await serp_1.default("site:quizlet.com " + info.selectionText);
    const ids = results.map(result => quizlet_1.getId(result.link)).filter(it => it);
    const sets = await quizlet_1.getSets(ids);
...
function injectCodeIfNeeded(tab) {
    chrome.tabs.sendMessage(tab.id, null, async (e) => {
        if (!e)
            await injectCode();
    });
}
async function sendDb(tab, sets) {
    fuzzy_1.prepareSearch(sets);
}
chrome.runtime.onMessage.addListener(handleMessage);
function handleMessage(message, sender, respond) {
    const results = fuzzy_1.searchFuzzy(message);
...
exports.cache = {
    ...
    set(key, data) {
        return new Promise(res => chrome.storage.local.set({ [key]: data }, res));
    }
};
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.searchFuzzy = exports.prepareSearch = void 0;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.getSets = exports.scrapeSet = exports.getId = void 0;
...
async function scrapeSet(id) {
    ...
    cache_1.cache.set(id, simplified);
    return simplified;
}
exports.scrapeSet = scrapeSet;
async function getSets(ids, scrapeNew = false) {
    let sets = await cache_1.cache.get(ids);
    if (!scrapeNew)
        return sets;
